...
WorldEpisode treats simulators as runtime targets behind adapters, not as privileged storage
formats. A simulator adapter should implement three layers:

- `META-SIM.001` invariant interface: ingest world revisions, frame/clock graphs, entities,
  representation roles, actions, assets, and quality records.
- `META-SIM.002` asynchronous schema extension: register simulator-specific cloth, fluids,
  deformables, contact models, procedural assets, or articulation events without weakening the core
  sidecar.
- `META-SIM.003` deterministic replay accountability: emit runtime version, solver, timestep,
  actuator parameters, command/effective timestamps, latency model, initialization state, tolerance
  envelope, and measured divergence.

The current adapter matrix is generated by `tools/meta_simulator_contract.py`. WorldEpisode has
tested same-trace MuJoCo and Genesis replay adapters; URDF Studio separately tests MuJoCo and
Genesis against a shared episode-backend conformance contract and a one-episode carton-sorting
comparison; Isaac has an emitted but untested adapter mapping; SAPIEN remains an adapter-required
target. The claim is adapter compliance, not simulator-independent physics.
...
control-loop contract, and verifies timestamp-aware replay in MuJoCo and Genesis. The same
contract contains an Isaac adapter mapping, but that backend is marked ready and untested until an
Isaac environment is available.
