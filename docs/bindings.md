...
The current adapter matrix is generated by `tools/meta_simulator_contract.py`. MuJoCo has one tested
minimal replay adapter, Isaac has an emitted but untested adapter mapping, and Genesis/SAPIEN are
explicit adapter-required targets. The claim is adapter compliance, not simulator-independent
physics.
