...
The current adapter matrix is generated by `tools/meta_simulator_contract.py`. WorldEpisode has one
tested MuJoCo replay adapter; URDF Studio separately tests MuJoCo and Genesis against a shared
episode-backend conformance contract and a one-episode carton-sorting comparison; Isaac has an
emitted but untested adapter mapping; SAPIEN remains an adapter-required target. The claim is
adapter compliance, not simulator-independent physics.
