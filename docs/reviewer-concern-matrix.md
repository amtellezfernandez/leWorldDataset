...
| Replay experiment is narrow. | One lag is selected from 320 SO-101 calibration trajectories and evaluated on 80 source-episode-disjoint trajectories across eight tasks with paired bootstrap intervals. A separately preregistered 32-scenario primitive contact protocol runs straight-push and parallel-jaw-capture tasks in MuJoCo and Genesis with pose, contact, grasp, and outcome intervals. | Mitigated for repeated telemetry alignment and primitive contact execution. The timing source still has no motor-effective timestamps or second controller; contact actors are scripted primitives with no hardware ground truth. | Instrument command/queue/motor timestamps on a second controller and anchor articulated contact replay to hardware observations. |
| Runtime-neutral claim overstates evidence. | `tools/meta_simulator_contract.py` defines three adapter compliance layers; MuJoCo and Genesis execute same-trace and primitive contact protocols, Isaac is ready/untested, and SAPIEN is adapter-required. | Mitigated for measurable cross-runtime drift. Contact F1 and outcomes are high, but substantial capture-orientation disagreement directly prevents an equal-physics claim. | Add articulated robot tasks, hardware-reference trajectories, and Isaac/SAPIEN runtime reports before claiming broader simulator neutrality. |
...
| Famous benchmark impact is not demonstrated. | `tools/benchmark_callout_audit.py` audits Open X-Embodiment, DROID, BridgeData V2, LIBERO, and CALVIN for public leakage/timing controls. `tools/famous_benchmark_policy_rerun.py` executes a pinned DROID subset conversion and bounded offline policy rerun, and `tools/benchmark_inflation_gate.py` separately requires benchmark-specific conversion, split/timing audit, published-protocol evidence, and policy-rerun artifacts before any score-inflation claim. Current measured famous-benchmark inflation claims: 0. | Open but fail-closed. The DROID subset run is real, but not inflation-proof: lineage is only a task/camera-schema proxy, the protocol is not a published leaderboard reproduction, and the corrected proxy split does not reduce the score. The paper can say one bounded DROID rerun executed and that the proof gate currently has no inflation-proof rerun reports; it must not say published scores are inflated. | Convert at least one famous benchmark into WorldEpisode with physical scene/source lineage and rerun a published protocol under lineage-disjoint splits or timestamp-aware replay. |
...
4. **Famous benchmark call-out gate**
   ...
   - Targeted rerun tool: `tools/famous_benchmark_policy_rerun.py --benchmark droid_100`.
   - Proof gate: `docs/experiments/benchmark_inflation_gate/gate_report.json` records five
     required tests, one executed DROID subset rerun artifact, and zero inflation-proof
     famous-benchmark rerun reports. Reproduce the blocking gate with `python3 tools/benchmark_inflation_gate.py
     --required`; it should fail until rerun evidence is committed.
...
5. **Contact-rich replay gate**
   ...
   - Current output: `docs/experiments/meta_simulator_contract/adapter_contract_report.json`
     ...
     queue semantics.
   - Required output: contact-rich task replay across at least two simulators, with contact/event
     metrics, task outcome agreement, and declared tolerance envelopes.
   - Claim unlocked: replay assumptions transfer beyond minimal joint tracking.
...
8. **Generalization-beyond-robotics evidence gate**
   ...
   - Claim unlocked: the WorldEpisode invariant vocabulary is empirically useful outside robotics,
     not only a general vocabulary claim.
