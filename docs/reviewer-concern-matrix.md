...
| Famous benchmark impact is not demonstrated. | `tools/benchmark_callout_audit.py` audits Open X-Embodiment, DROID, BridgeData V2, LIBERO, and CALVIN for public leakage/timing controls. `tools/famous_benchmark_policy_rerun.py` now attempts a pinned DROID subset conversion and offline policy rerun, and `tools/benchmark_inflation_gate.py` separately requires benchmark-specific conversion, split/timing audit, published-protocol evidence, and policy-rerun artifacts before any score-inflation claim. Current measured famous-benchmark inflation claims: 0. | Open but fail-closed. The attempted DROID run is invalid in this checkout because the local rerun environment could not satisfy the public-data/dependency path before the required Parquet shards were read. The paper can say these benchmarks require targeted WorldEpisode audits and that the proof gate currently has no valid rerun reports; it must not say their scores are inflated. | Re-run the DROID subset tool with working HF access and experiment dependencies, then convert at least one famous benchmark into WorldEpisode and rerun a published protocol under lineage-disjoint splits or timestamp-aware replay. |
...
4. **Famous benchmark call-out gate**
   ...
   - Proof gate: `docs/experiments/benchmark_inflation_gate/gate_report.json` records five
     required tests, one attempted DROID subset rerun artifact, and zero valid famous-benchmark
     rerun reports. Reproduce the blocking gate with `python3 tools/benchmark_inflation_gate.py
